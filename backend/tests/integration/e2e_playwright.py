...
集成测试脚本（需要先启动后端服务）：
    python backend/tests/integration/e2e_playwright.py

说明：
- 命名不带 `test_` 前缀，避免被 pytest 自动收集
- 需要先启动后端：python backend/run.py
- 需要安装 Playwright 浏览器：playwright install chromium
- 鉴权需为 demo 模式（默认），或修改下方 AUTH 头
"""
...
def main():
    ...
    # 从 backend/tests/integration/ 回到项目根目录：上溯三级
