...
// 1. 代理调用 OpenAI 兼容接口（如 DeepSeek），避免侧边栏/内容脚本的 CORS 问题
// 2. 代理 OfferClaw 后端 API 请求，避免 content script 的 CORS 问题

importScripts("shared/resume/model-storage.js");
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request?.action === "backendRequest") {
    backendProxy(request.method, request.path, request.body)
      .then((response) => sendResponse({ success: true, data: response }))
      .catch((error) =>
        sendResponse({ success: false, error: error?.message || String(error) })
      );
...
不要返回未列出的 transform。`,
  ...
  if (!response.ok) {
    ...
    console.error("[OfferClaw] AI 请求失败:", {
...
// ─ 后端代理 ─────────────────────────────────────────────
async function backendProxy(method, path, body) {
  ...
  try {
    ...
    clearTimeout(timeoutId);
    if (err.name === "AbortError") {
      throw new Error("后端请求超时（8s）：请检查后端是否已启动");
    }
    if (err.message && err.message.includes("Failed to fetch")) {
      throw new Error(`无法连接后端（${backendUrl}）：请确认服务已启动`);
    }
    throw err;
  }
}
...
async function getBackendUrl() {
  try {
    ...
    return (data.config?.backend || "http://localhost:8000").replace(/\/+$/, "");
  } catch (e) {
